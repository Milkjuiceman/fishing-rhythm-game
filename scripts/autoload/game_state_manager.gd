...
func add_currency(amount: int) -> void:
	current_save_data.currency += amount
	print("[GameStateManager] Added currency: %d → Total now: %d" % [amount, current_save_data.currency])
	player_state_changed.emit(current_save_data)
	
func add_item(item_name: String, amount: int = 1) -> void:
	if not current_save_data.inventory.has(item_name):
		current_save_data.inventory[item_name] = 0
	current_save_data.inventory[item_name] += amount
	print("[GameStateManager] Added item '%s': %d → Total now: %d" % [
		item_name, amount, current_save_data.inventory[item_name]])
	player_state_changed.emit(current_save_data)

func add_fish(fish_name: String, amount: int = 1) -> void:
	if not current_save_data.fish_caught.has(fish_name):
		current_save_data.fish_caught[fish_name] = 0
	current_save_data.fish_caught[fish_name] += amount
	print("[GameStateManager] Caught fish '%s': %d → Total now: %d" % [
		fish_name, amount, current_save_data.fish_caught[fish_name]])
	player_state_changed.emit(current_save_data)
